...
def df_processor(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Loại bỏ tzinfo nếu có
    tz_cols = df.select_dtypes(include=["datetimetz"]).columns
    for col in tz_cols:
        df.loc[:, col] = df[col].dt.tz_localize(None)
    # Chuẩn hóa các cột object ngoại trừ 'Ngày'
    for col in df.select_dtypes(include=["object"]).columns:
        if col != "Ngày":
            df.loc[:, col] = df[col].apply(
                lambda x: x.strftime("%Y-%m-%d") 
                if isinstance(x, (pd.Timestamp, datetime.date)) and not pd.isnull(x) 
                else x
            )
...
def store_dataframe_in_mongodb(df: pd.DataFrame,
    ...
    # Chuẩn hóa dữ liệu: chuyển datetime về string, None về null
    records = []
    for rec in df.to_dict(orient="records"):
        # Nếu trường Ngày là datetime.date hoặc Timestamp, giữ nguyên; else để string/case empty
        if isinstance(rec.get("Ngày"), (pd.Timestamp, datetime.date)):
            rec["Ngày"] = rec["Ngày"].isoformat()
        records.append(rec)
...
def xlsx_processor_pipeline(file_path: str, mongo_uri: str, db_name: str = "myDatabase"):
    ...
    # Đổi tên 13 cột đầu theo thứ tự
    ...
    df.columns = cols_13 + list(df.columns[13:])  # giữ nguyên các cột khác

    # Chuỗi pipeline xử lý
    df = df_processor(df)
    ...
    # Lưu lên MongoDB
    store_dataframe_in_mongodb(df, mongo_uri, db_name=db_name, collection_name="hs_code")
